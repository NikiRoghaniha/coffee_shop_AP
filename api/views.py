...
from api.serializers import UserSerializer, OTPLoginSerializer, OTPVerificationSerializer, CategorySerializer, \
    ...
    AddToCartSerializer, OrderSerializer, PayOrderSerializer, ProductSalePerDaySerializer
...
class SaleReportView(generics.ListAPIView):
    serializer_class = ProductSalePerDaySerializer
    permission_classes = [IsAdminUser]
    ...
    def get_queryset(self):
        ...
        return queryset.annotate(
            sale_count=Sum('orderitem__quantity', filter=Q(orderitem__order__is_paid=True))
        ).annotate(day=TruncDay('orderitem__order__created_at')).values('day', 'sale_count').order_by('day')
